...
| --- | --- | --- |
| 转换语音、歌曲或视频 | 可回退的转换工程、逐段多音色时间线、成品处理链和结果版本 | 转换工程 |
| 用麦克风实时变声 | 可复用场景、虚拟声卡路由、全局控制及干声/湿声双轨录音 | 实时变声 |
| 处理整个目录或持续接收新文件 | 带预检、命名模板、过滤与冲突策略的可恢复共享任务 | 批量与监控 |
| 接入脚本或外部 Agent | 可发现的本机 HTTP/WebSocket、CLI 和 stdio MCP 合同 | CLI、API 与 MCP |
...
转换工作台接受 WAV、FLAC、MP3、MP4 和 MKV。语音模式可以先分析说话人，只转换选中的说话人；转换工程会保存波形、分段边界、逐段启用状态、音色与参数，并提供拆分、合并、撤销、重做和历史版本恢复。一个工程可把不同片段分配给不同模型，实际渲染时按模型分组复用加载结果。歌曲模式可以在用户自行准备分离模型后处理人声并混回伴奏；最多四组参数可以生成同步 A/B 试听，模型库还可以对 2–8 个音色生成同源同参对比。

预设可以复制、更新、归档、恢复、导入和导出。模型库可维护封面、自定义名称、标签、收藏、备注和试听样本，并显示使用次数、最近使用时间、重复权重和完整性检查结果。成品处理链包含去混响、降噪、低切、低频/清晰度均衡、压缩、齿音控制、目标响度、限幅与首尾静音处理。长音频会在低能量位置分块，但只加载一次 RVC 模型。视频输出复制原视频流和原音轨，再新增命名后的变声音轨；最终发布前会完成媒体解码、清单和哈希检查。每次成品都会进入带父子关系的结果版本记录，可查看参数与输出差异、做 A/B 播放，并用当时的输入和模型修订精确重跑。

### 实时麦克风

在“设置与诊断”中选择 Windows 音频接口、麦克风和播放设备，再到“实时变声”选择模型与参数。输入和输出必须属于同一个音频接口；连续模式建议使用耳机，避免播放声音被麦克风再次收录。

实时页提供 0.25、0.5 和 1.0 秒三档延迟预算。麦克风校准会实测噪声底、信噪比、音高范围和设备稳定度，再推荐噪声门、VAD 阈值、音高、索引率和延迟档位；路由测试会实际打开指定入出设备完成一次闭环探测，而不只检查设备名称。Silero VAD 和用户设置的麦克风启动阈值共同决定何时进入推理；测试模式会先录入并转换整句话，在停顿后播放，适合在开始连续监听前检查效果。场景按设备名称保存模型、路由、参数、录音开关和快捷键，设备编号在重启后变化也会重新匹配。

连续会话中可以即时切换原声旁路、静音、录音和按键说话，不需要重启模型；悬浮迷你面板与系统托盘在主窗口隐藏时仍可完成这些控制。默认全局快捷键为 `Ctrl+Alt+F9/F10/F11/F12`，依次控制开始/停止、旁路、静音和按住说话。录音写入数据目录的 `artifacts/realtime`，分别生成麦克风干声和最终湿声 WAV，同时保存包含文件哈希、模型修订和运行指标的录音清单；停止后可把干声和当时的模型参数直接提升为离线转换工程。路由助手会识别 VB-CABLE 与 Voicemeeter。

实时会话和离线任务共用 GPU：已有离线任务运行时不能启动实时会话；实时会话开始后，新任务会保持排队，并在会话停止后继续。预热会暂停离线任务，因此不会在后台自动发生；用户可以明确点击“准备模型”，也可以直接开始实时会话并等待预热完成。离开实时页且没有活动会话时会释放驻留工作进程和 GPU 显存。

### 批量与持续监控

批量规则把输入目录、输出目录、多组模型/预设变体和监控状态保存到数据库。每个变体可单独设置扩展名与包含/排除 glob，同一个输入可以一次生成多个模型结果；多变体即使命名模板没有写 `{variant}` 也会得到不同目标路径。规则可以编辑、归档和恢复，归档不会删除规则、任务或文件。运行前可以预检文件数、输出数、体积、目标路径和冲突；命名模板支持 `{stem}`、`{source_ext}`、`{variant}`、`{model}`、`{preset}`、`{hash}`，还能选择是否保留目录结构、输出格式以及跳过、版本化或覆盖策略。新文件写入稳定后才会进入队列；输出目录会从输入枚举中排除，内容 SHA-256 用于去重。单个失败项可以更换模型、预设和输出路径后单独重试，不必重跑整批。

任务可以取消、重试并在任务中心统一查看。设置页会分别统计结果、中间产物、失败运行、模型、缓存与组件占用；归档后的任务可以校验后恢复。迁移数据目录时，程序会先生成带摘要的计划，退出服务后复制并逐文件核对，切换数据目录指针再重启，旧目录保留为回退来源。VoxWeave 不会自动删除中间产物。更新页只读取项目的 GitHub Release，Windows ZIP 必须带有发布者 SHA-256 摘要才允许下载；新版本并排安装，激活后必须在期限内报告健康，否则启动器自动回到上一版本。
...
所有请求使用 `voxweave-control v1`。会修改状态的操作和长任务都需要全局唯一的 `request_id`；网络中断后使用同一 ID、操作、参数和调用者重发，会得到第一次调用的结果而不会重复执行。长任务立即返回 `task_id`，随后用 `task get` 查询，或连接发现文件声明的已认证本机 WebSocket：
...
外部 Agent 也可以直接启动 stdio MCP 服务，不需要在 VoxWeave 内增加聊天界面或模型运行时：

```powershell
python -m voxweave.mcp_server
```

MCP 服务会从正在运行的本机服务读取最新 `describe` 合同，把每个公开操作动态暴露为 `voxweave_<操作名>` 工具；长任务仍返回持久化 `task_id`，Agent 随后调用 `voxweave_task_get` 检查结果。客户端配置与调用约束见 [外部 Agent 与 MCP](docs/MCP.md)。

## 数据、模型与边界

- SQLite 是工程与修订、结果版本、模型、预设、任务、批量规则、实时会话与场景、事件、产物及归档记录的唯一状态真源。
