...
实时变声页会列出 RVC 运行环境实际识别的 Windows 音频接口、麦克风和播放设备，并优先选择 Windows WASAPI 的默认输入和输出；WASAPI 不可用时才回退到其它接口。麦克风按单声道采集，Silero VAD 负责判断人声；当 VAD 漏检但实际输入超过静音电平时，音频仍会进入 RVC，只有真正的静音块才向播放设备写入零值。页面会显示语音检测、变声输出以及输入/输出电平。默认 0.5 秒音频块；性能不足时可切到 1.0 秒稳定档，增加延迟来换取连续性。打开测试模式后，每段变声结果播放时会丢弃同期麦克风输入，避免扬声器外放重新进入模型；普通连续模式仍建议使用耳机。输入和输出必须属于同一个音频接口。自动化调用依次使用 `realtime.devices`、`realtime.start`、`realtime.status` 和 `realtime.stop`。实时会话不占离线任务线程，但会独占 GPU：已有任务运行时不能启动，之后提交的任务保持排队并在实时会话停止后继续。
