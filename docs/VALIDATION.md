...
本轮按原性能审计的八项问题逐项复验，没有打包或删除文件。离线链使用正式数据根、`local.public-yujie-v2.default` 和同一段 5.4 秒真实 WAV，经公开认证服务完成转换。冷任务包含 RVC 进程启动与模型加载，共 20.483 秒；同模型热任务为 3.560 秒，整改前同条件热任务为 7.294 秒。输出为 777702 字节 PCM WAV，完整解码、音频质量检查和 `voxweave-conversion-result v1` 校验全部通过，不再出现 `invalid_result`。

常驻服务启动后工作集/私有内存为 56.7/41.5 MiB，真实转换结束后立即为 59.4/44.0 MiB。NumPy、SciPy 和 SoundFile 后处理现在只在可取消的短进程中加载，退出后不再把约 1 GiB 提交内存留在服务。任务内模型、索引、输入和输出哈希绑定到实际文件身份；同一身份只完整读取一次，身份变化会使凭据失效。5.4 秒热任务的缩短包含了这项收益，下载测试也确认写入时同步累计 SHA-256，不再回读刚完成的临时文件。

200 个近似说话人嵌入的全局平均链接聚类完成于 0.180 秒，只计算 39601 个候选对；原实现同规模约为 30 秒量级。SQLite 复用连接后，500 次空任务事件查询耗时 0.0058 秒，中位数 0.0106 毫秒；整改前为 1.252 秒。WebSocket 已改为任务事件条件唤醒，桌面端完全空闲时停止实时状态定时器；连续 100 次状态查询没有产生 INFO 日志字节。

实时链先显式完成模型预热，再经正式 `realtime.start/status/stop` 使用 Windows WASAPI 默认麦克风和扬声器运行 0.25 秒档 48 个回调。会话进入 running 并正常 stopped，预计缓冲延迟 540 毫秒，PortAudio xrun 为 0、过载为 0；环境输入低于门限，所以这次没有伪造语音推理数据。历史真声链的单次推理为 132–152 毫秒，低于 0.25 秒预算。新配置因此默认选择 0.25 秒档，已有用户明确保存的偏好保持不变。

最终源码共收集 253 项测试，251 项通过、2 项按环境条件跳过；Ruff、`compileall` 和 `git diff --check` 通过。结构测试还确认主服务媒体模块不再包含 NumPy、SciPy、SoundFile 或逐块单次转换的旧路径，任务专用事件唤醒状态在等待连接结束后会被回收。加载最终代码重启服务后，空闲工作集/私有内存为 59.4/44.4 MiB；100 次 `realtime.status` 共 0.952 秒，INFO 日志仍为零增长。
